...
final class ProfileStore: ObservableObject {
    ...
    private let profileKey = "bymy.userProfile.v2"
    private let tokenKey = "bymy.authToken.v1"
    private let userIdKey = "bymy.userId.v1"
    /// Régi Add el autod kulcsok — egyszeri átvétel.
    private let legacyProfileKey = "addelautod.userProfile.v2"
    private let legacyTokenKey = "addelautod.authToken.v1"
    private let legacyUserIdKey = "addelautod.userId.v1"
    ...
    private func migrateLegacyDefaultsIfNeeded() {
        ...
        if d.data(forKey: profileKey) == nil, let legacy = d.data(forKey: legacyProfileKey) {
            d.set(legacy, forKey: profileKey)
        }
        if d.string(forKey: tokenKey) == nil, let legacy = d.string(forKey: legacyTokenKey) {
            d.set(legacy, forKey: tokenKey)
        }
        if d.integer(forKey: userIdKey) == 0 {
            let legacyId = d.integer(forKey: legacyUserIdKey)
            if legacyId > 0 { d.set(legacyId, forKey: userIdKey) }
        }
    }

 ...
 *
 * Használat:
 *   node addelautod-ios/scripts/sync-vehicle-catalog.mjs
...
const OUT = join(ROOT, "AddElAutod", "VehicleCatalog.json");
...
function main() {
  ...
  if (arg) {
    ...
    if (!csv) {
      ...
      console.error("Használat: node addelautod-ios/scripts/sync-vehicle-catalog.mjs ~/Desktop/lista.csv");
      process.exit(1);
    }

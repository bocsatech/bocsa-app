...
const BEWEGUNG_COLUMNS_FULL =
  "id, created_at, lager_teil_id, menge, typ, richtung, machine_id, fahrzeug_id, arbeitsauftrag_id, referenz, bemerkung";
...
function isSchemaExtensionError(message) {
  return /typ|richtung|fahrzeug_id|arbeitsauftrag_id|column/i.test(String(message ?? ""));
}
...
export async function insertLagerBewegung(db, row) {
  ...
  if (row.arbeitsauftrag_id) {
    full.arbeitsauftrag_id = row.arbeitsauftrag_id;
  }

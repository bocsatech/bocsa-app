#!/bin/bash
# Egy parancs: feature ág + másolás a futó Bymy mappába + ellenőrzés
set -euo pipefail
...
echo "══════════════════════════════════════"
echo " Bymy járműkatalógus telepítés"
echo "══════════════════════════════════════"
echo "  Repo:   $REPO"
echo "  Ág:     $BRANCH"
echo "  Cél:    $TARGET"
...
git stash push -u -m "bymy-katalogus-telepit-$(date +%Y%m%d-%H%M%S)" 2>/dev/null || true
...
cp "$MAC_DIR/Bymy-indito.command" "$HOME/Desktop/Bymy-indito.command" 2>/dev/null || true
chmod +x "$HOME/Desktop/Bymy-indito.command" 2>/dev/null || true
...
echo "2) Nyisd:   https://bymy.vercel.app/hirdetesfeladas.html"
echo "3) Cmd+Shift+R"
echo ""

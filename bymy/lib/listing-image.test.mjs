...
test("listing-image: tartós útvonal + resolve", async () => {
  const dir = mkdtempSync(join(tmpdir(), "bymy-up-"));
  process.env.BYMY_UPLOADS_PATH = join(dir, "listings");
  ...
  delete process.env.BYMY_UPLOADS_PATH;
});

test("isListingImageMissing / displayImageUrl: https → proxy", async () => {
  const dir = mkdtempSync(join(tmpdir(), "bymy-up2-"));
  process.env.BYMY_UPLOADS_PATH = join(dir, "listings");
...
test("listing-image forrás: ~/.bymy", () => {
  const src = readFileSync(join(__dirname, "listing-image.mjs"), "utf8");
  assert.match(src, /\.bymy/);
  assert.match(src, /BYMY_UPLOADS_PATH/);

...
import { bymyHomeDir, bymyDbFileName, envPath, ensureBymyHomeDir } from "./data-home.mjs";

const __dirname = dirname(fileURLToPath(import.meta.url));
const DATA_DIR = join(__dirname, "..", "data");

/** App mappán kívül — túléli a Downloads/bymy frissítést / újratelepítést. */
function stableHomeDbPath() {
  ensureBymyHomeDir();
  return join(bymyHomeDir(), bymyDbFileName());
}

function legacyAppDbPath() {
  const next = join(DATA_DIR, "bymy.db");
  const legacy = join(DATA_DIR, "autosweb.db");
...
function resolveDbPath() {
  const fromEnv = envPath("BYMY_DB_PATH", "AUTOSWEB_DB_PATH");

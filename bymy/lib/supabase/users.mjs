...
function profileFromRow(row) {
  ...
  const merged = {
    ...
    country: String(row.country ?? "").trim() || "Magyarország",
...
function profileToColumns(profile, { preserve } = {}) {
  const next = {
  ...
  const avatarDataUrl =
    profile.avatarDataUrl != null
      ...
      : preserve?.avatar_data_url ?? null;

  let pageLayout = preserve?.page_layout ?? null;
  if (profile.pageLayout != null) {
    pageLayout =
      typeof profile.pageLayout === "string"
        ? profile.pageLayout
        : JSON.stringify(profile.pageLayout);
  }

  const searchRadiusKm =
    profile.searchRadiusKm != null
      ? Number(profile.searchRadiusKm)
      : preserve?.search_radius_km ?? null;
  ...
  return {
    ...
    columns: {
      ...
      search_radius_km: Number.isFinite(searchRadiusKm) ? searchRadiusKm : null,
...
export async function mergeUserProfileJson(userId, patch = {}) {
  ...
  const current = profileFromRow(row);
  const merged = { ...current, ...patch };
  const { columns } = profileToColumns(merged, { preserve: row });
...
export async function saveUserProfile(userId, profile) {
  ...
  // Avatar / layout megőrzés, ha a PUT nem küldi őket.
  if (profile.avatarDataUrl === undefined && previous.avatarDataUrl) {
    incoming.avatarDataUrl = previous.avatarDataUrl;
  }
  if (profile.pageLayout === undefined && previous.pageLayout != null) {
    incoming.pageLayout = previous.pageLayout;
  }

  const { profile: next, columns } = profileToColumns(incoming, { preserve: row });
  ...
  const { error } = await sb()
    ...
    .eq("id", userId);
  if (error) throw error;
  return profileFromRow({ ...row, ...columns, display_name: displayName });
}
...
export async function inspectWebUsersDb() {
  ...
  const mapped = (users ?? []).map((row) => {
    ...
    // Ne küldjük ki a base64 avatárt a debug listában.
    const safe = { ...profile };
    if (safe.avatarDataUrl) safe.avatarDataUrl = `[data-url ${safe.avatarDataUrl.length} chars]`;
    return {
      id: row.id,
      email: row.email,

...
const ATTACH_DIR = process.env.VERCEL || process.env.AWS_LAMBDA_FUNCTION_NAME
...
export async function initMessagingSchema() {
  try {
    ...
    console.warn("ensureDemoSeller:", error.message ?? error);
  }
  try {
    if (!existsSync(ATTACH_DIR)) mkdirSync(ATTACH_DIR, { recursive: true });
  } catch (error) {
    // Vercel/home nem írható — listázás/üzenetek DB-ből mennek, csatolmány később
    console.warn("ATTACH_DIR mkdir skipped:", error.message ?? error);
  }
}
...
function saveAttachment(conversationId, attachment) {
  ...
  try {
    ...
    const err = new Error("Csatolmány mentése nem elérhető ezen a szerveren.");
    err.status = 503;
    throw err;
  }

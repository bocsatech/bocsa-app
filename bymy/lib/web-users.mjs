/** Bymy web userek — SQLite, nem Supabase. */
...
const SESSION_COOKIE = "bymy_session";
const LEGACY_SESSION_COOKIE = "autosweb_session";

export { SESSION_COOKIE, LEGACY_SESSION_COOKIE, getProfilesFilePath, ensureProfilesStore };
...
function linkIdentity(userId, { provider, subject, email = "", profile = {} }) {
  ...
  if (existing) {
    if (Number(existing.user_id) !== Number(userId)) {
      throw new Error("Ez a social fiók már másik Bymy felhasználóhoz van kötve.");
    }
    db.prepare(
...
export function getSessionTokenFromRequest(req) {
  ...
  if (cookies[LEGACY_SESSION_COOKIE]) return cookies[LEGACY_SESSION_COOKIE];

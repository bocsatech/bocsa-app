/** OAuth (Google / Apple / Facebook) — config: ~/.bymy (vagy ~/.autosweb)/oauth.json */
...
const EXAMPLE = {
  publicBaseUrl: "https://bymy.vercel.app",
  stateSecret: "",
  google: {
    enabled: false,
    clientId: "",
  ...
  apple: {
    ...
    teamId: "",
    keyId: "",
    privateKeyPath: "~/.bymy/AuthKey_XXXXX.p8",
  },
};

export function oauthConfigPath() {
  const fromEnv = envPath("BYMY_OAUTH_PATH", "AUTOSWEB_OAUTH_PATH");
  if (fromEnv) return fromEnv;
  return join(bymyHomeDir(), "oauth.json");
}
...
export function loadOAuthConfig() {
  ...
  try {
    ...
    return {
      publicBaseUrl: String(raw.publicBaseUrl || "https://bymy.vercel.app").replace(/\/$/, ""),
...
export function callbackUrl(provider, cfg = loadOAuthConfig()) {
  const base = cfg?.publicBaseUrl || "https://bymy.vercel.app";
...
export function buildAuthorizeUrl(provider, state, cfg = loadOAuthConfig()) {
  if (!isProviderConfigured(provider, cfg)) {
    throw new Error(`${provider} OAuth nincs beállítva (~/.bymy/oauth.json).`);
  }

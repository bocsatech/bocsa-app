/** Fő kép letöltés — tartós mappa: ~/.bymy (vagy ~/.bymy)/uploads/listings/ */

import { copyFileSync, existsSync, mkdirSync, readdirSync, unlinkSync, writeFileSync } from "fs";
import { join, dirname } from "path";
import { fileURLToPath } from "url";
import { bymyHomeDir, envPath } from "./data-home.mjs";

const __dirname = dirname(fileURLToPath(import.meta.url));
const LEGACY_UPLOAD_DIR = join(__dirname, "..", "public", "uploads", "listings");

function stableUploadDir() {
  const fromEnv = envPath("BYMY_UPLOADS_PATH", "AUTOSWEB_UPLOADS_PATH");
  if (fromEnv) return fromEnv;
  return join(bymyHomeDir(), "uploads", "listings");
}
...
/** Törli a helyi hirdetésképeket (~/.bymy vagy ~/.bymy uploads/listings). */

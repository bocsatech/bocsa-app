...
function serveStatic(path, res) {
  ...
  // Hirdetésképek: ~/.bymy/uploads (túléli a frissítést)
...
server.listen(PORT, HOST, async () => {
  ...
  console.log(`Bymy (helyi): http://${HOST}:${PORT}`);
  console.log(`Bymy (élő): https://bymy.vercel.app`);
  ...
  if (isSmtpConfigured()) {
    ...
    console.warn(
      `SMTP: NINCS — másold ~/.bymy/smtp.example.json → smtp.json (Gmail app jelszó). Futtasd: bymy/mac/smtp-beallitas.command`
    );
  }
  ...
  if (enabledOauth.length) {
    ...
    console.warn(
      `OAuth: nincs aktív provider — szerkeszd: ${oauthConfigPath()} (példa: ~/.bymy/oauth.example.json). Futtasd: bymy/mac/oauth-beallitas.command`
    );
  }

...
struct PostAdTruckScreen: View {
    ...
    private var rakterBody: some View {
        VStack(spacing: 0) {
            ...
            Divider().padding(.leading, 16)
            numberRow("Raktér hossza", placeholder: "mm", focus: .rakterH, binding: intBinding(\.rakterHosszTol, \.rakterHosszIg, grouped: true))
            Divider().padding(.leading, 16)
            numberRow("Raktér szélessége", placeholder: "mm", focus: .rakterSz, binding: intBinding(\.rakterSzelessegTol, \.rakterSzelessegIg, grouped: true))
            Divider().padding(.leading, 16)
            numberRow("Raktér magassága", placeholder: "mm", focus: .rakterM, binding: intBinding(\.rakterMagassagTol, \.rakterMagassagIg, grouped: true))
            Divider().padding(.leading, 16)
            numberRow("Doblemez-távolság", placeholder: "mm", focus: .doblemez, binding: intBinding(\.doblemezTavolsagTol, \.doblemezTavolsagIg, grouped: true))
        }
    }
